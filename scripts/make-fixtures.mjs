 ...
 *
 * 社名は甲野・乙川・丙山・丁田。日本の法律文書で当事者を指すときに使う
 * 架空名なので、読む人にひと目で「実在しない」と分かる。
 * 電話番号は市外局番＋1234-5678。人名は山田・田中・佐藤・鈴木。
 * どれも、実在の会社や個人を指さないことが形から分かるものを選んでいる。
...
const ROWS = [
  ...
  ['株式会社甲野商事', '田中 一郎', '03-1234-5678', '100-0001', '12,000', '2026/1/15', '007'],
  ['(株)甲野商事', '佐藤 花子 ', '0312345678', '1000001', '￥8,500', '2026-01-20', '015'],
  ['㈱甲野商事', '鈴木　次郎', '03(1234)5678', '〒100-0001', '3000円', '令和8年1月25日', '023'],
  ['乙川物産株式会社', '', '06-1234-5678', '530-0001', '45,000', '2026/2/1', '101'],
  ['乙川物産㈱', '高橋 三郎', '０６-１２３４-５６７８', '530-0002', '45000', '2026/02/01', '102'],
  ['丙山工業', '伊藤 四郎', '045-000-0000', '220-0011', '7,800', '2026/2/10', '205'],
  ['丙山工業 ', '渡辺 五郎', '045-000-0000', '220-0011', '7,800', '2026/2/10', '205'],
  ['丁田商店', '山本 六郎', '011-000-0000', '060-0001', '999999999', '2026/3/3', '310'],
]
...
for (const n of sizes) {
  const parts = [header]
  for (let i = 1; i <= n; i++) {
    const variant = i % 3 === 0 ? '(株)甲野商事' : i % 3 === 1 ? '株式会社甲野商事' : '㈱甲野商事'
    parts.push(
